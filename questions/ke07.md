...
### Auf welcher Sprachebene wird *Elimination toten Codes* angewendet?
...
### Auf welcher Sprachebene wird *Code Hoisting* angewendet?
...
### Auf welcher Sprachebene wird *Anweisungsreihenfolge und Registerauswahl* angewendet?
...
### Auf welcher Sprachebene wird *Befehlsauswahl* angewendet?

3AC $\rightarrow$ M M $\rightarrow$ M
...
### Auf welcher Sprachebene wird *Peephole Optimization* angewendet?
...
1. Zu Beginn des ersten Blocks gibt es keine gültigen Definitioen (bzw. die der Eingabeparameter
   einer Prozedur) $$ in(B_1) = \emptyset $$ 
...
1. Am Ende des Programms leben keine Variablen mehr $$ out(B_n) = \emptyset $$ 
...
3. Am Anfang eines Blocks leben alle Variablen, die vor ihrer Definition im Block verwendet werden
   ...
   definierten Variablen sind also Teil von $kill(B_i)$. $$ in(B_i) = gen(B_i) \cup (out(B_i)
   \setminus kill(B_i)) $$

### Für welche Optimierung kann die backward-all-Datenflussanalyse genutzt werden?

Identifikation von very-busy-Ausdrücken, also solchen Ausdrücken (Variablen), deren Wert vor jeder
Redefinition genutzt wird (auf jedem möglichen Pfad)
TODO delete?
...
### Welche 4 Fallunterscheidungen zur Gestalt eines 3AC Statements werden in *codegen* getroffen?
